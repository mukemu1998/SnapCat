...
@(
    ...
    "## 版本定位"
    ""
    "v$version 是 SnapCat 的本地增强版，重点补齐自由框选后的实际使用体验：翻译浮窗、贴图管理、智能预框选、主题统一、多 API 配置和更完整的快捷键管理。"
    ""
    "## 重点更新"
    ""
    "- 自由框选体验升级：新增智能预框选，进入框选模式后可预选窗口、屏幕边缘和部分控件区域，单击即可确认。"
    "- 翻译浮窗重做：截图翻译后显示轻量浮窗，支持原文编辑、译文查看、语言选择、复制和再次框选识别。"
    "- 贴图能力增强：贴图支持拖动、滚轮缩放、翻转、阵列、复制并贴图、隐藏、分组管理和重启恢复位置。"
    "- 操作与快捷键整理：自由框选、保存、贴图显示/隐藏、打开主菜单、退出程序等操作统一纳入快捷键配置。"
    "- OCR 与翻译增强：内置本地 OCR 与本地轻量翻译，API 翻译支持多配置管理、命名、删除和上下文选项。"
    "- 主题与图标统一：新增多套深色主题，默认深海蓝主题、运行时主题图标和 EXE 图标色调保持一致。"
    "- 启动与托盘优化：启动时增加 logo 加载页，托盘菜单风格统一，并支持左键默认动作和贴图组显示。"
    "- 截图与历史管理：默认截图目录、临时文件清理、历史保留天数和截图缩略图管理进一步整理。"
    ""
    "## 使用建议"
    ""
